...
class AuthPage extends StatefulWidget {
  final String userType; // 'client', 'supplier', 'contractor', 'transporter', 'admin'
...
class _AuthPageState extends State<AuthPage> {
          ...
          child: ListView(
            children: [
              ...
              if (widget.userType != 'client')
                TextFormField(
                  decoration: const InputDecoration(labelText: 'اسم المؤسسة/الشركة'),
                  validator: (value) => widget.userType != 'client' && (value == null || value.isEmpty)
                  ...
                  children: [
                    TextFormField(
                      decoration: const InputDecoration(
                        ...
                        helperText: 'يجب أن تحتوي على حروف كبيرة وصغيرة وأرقام ورموز خاصة',
                      ),
                      obscureText: true,
              ...
              ElevatedButton(
                ...
                onPressed: () {
                  if (_formKey.currentState!.validate()) {
                    ...
                    if (widget.userType == 'client' || widget.userType == 'admin') {
                      ...
                      String? smsOtp;
                      if (widget.userType == 'client' && _phoneNumber != null && _phoneNumber!.isNotEmpty) {
                      ...
                      final user = User(
                        ...
                        name: name,
                        email: email,
                        phone: widget.userType == 'client' ? _phoneNumber : null,
                        type: widget.userType,
                        createdAt: DateTime.now(),
                        institution: widget.userType == 'admin' ? institution : null,
                      ...
                      _showOTPVerificationDialog(context, emailOtp, user, isEmailVerification: true);
                ...
                child: Text(widget.userType == 'client' ? 'تسجيل' : 'إرسال طلب العضوية'),
              ),
              const SizedBox(height: 8),
  ...
  Map<String, dynamic> _validateAndSanitizeInputs() {
    ...
    if (widget.userType != 'client') {
      ...
      if (_phoneNumber != null) {
        ...
        if (_phoneNumber!.isNotEmpty && !RegExp(r'^\+?\d{8,15}$').hasMatch(_phoneNumber!)) {
          isValid = false;
  ...
  void _submitMembershipRequest(BuildContext context) {
    ...
    final requestsManager = Provider.of<MembershipRequestsManager>(context, listen: false);
    requestsManager.addRequest(membershipRequest);
    
    // إظهار رسالة نجاح
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('تم إرسال طلب العضوية بنجاح. سيتم مراجعته والتواصل معك عبر البريد الإلكتروني.'),
  ...
  Future<Map<String, dynamic>> _sendVerificationSMS(String phoneNumber, String otp) async {
    ...
    try {
      final whatsappUrl = 'https://wa.me/$phoneNumber/?text=رمز التحقق الخاص بك هو: $otp';
    ...
    try {
      ...
      authManager.logSecurityEvent(
        ...
        {'phone': phoneNumber, 'success': true}
      );
  ...
  void _showOTPVerificationDialog(BuildContext context, String correctOTP, User user, {bool isEmailVerification = true}) {
                  ...
                  TextButton(
                    onPressed: remainingSeconds <= 0
                      ? () async {
                          ...
                          final result = await authManager.resendOTP(user.email);
                          ...
                          setState(() {
                            ...
                            if (result['success']) {
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content: Text(result['message'] ?? 'تم إعادة إرسال رمز التحقق'),
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content: Text(result['message'] ?? 'فشل إعادة إرسال رمز التحقق'),
                                  backgroundColor: Colors.red,
                                ),
                              );
                    ...
                    child: Text(
                      remainingSeconds > 0
                        ...
                        : 'إعادة إرسال الرمز',
                      style: TextStyle(
                        color: remainingSeconds > 0 ? Colors.grey : Theme.of(context).primaryColor,
                      ),
                    ),
                  ),
              ...
              actions: [
                TextButton(
                  onPressed: () {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text('لم يتم التحقق من حسابك. يمكنك تسجيل الدخول لاحقاً وإكمال عملية التحقق.'),
                ...
                TextButton(
                  onPressed: isLoading 
                    ...
                    : () async {
                      ...
                      final result = await authManager.verifyOTP(user.email, otpController.text);
                      ...
                      if (result['success'] == true) {
                        ...
                        if (isEmailVerification) {
                          ...
                          MockDatabase.updateUserVerificationStatus(user.id, true, 'email');
                          
                          // إذا كان العميل ويحتاج للتحقق من الهاتف أيضًا
                          if (user.type == 'client' && user.phone != null && !user.phoneVerified) {
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
                                content: Text('تم التحقق من البريد الإلكتروني. يرجى التحقق من رقم الهاتف الآن.'),
                                backgroundColor: Colors.green,
                              ),
                            );
                            ...
                            Future.delayed(const Duration(milliseconds: 1000), () {
                              if (!context.mounted) return;
                              _showOTPVerificationDialog(context, smsOtp, user, isEmailVerification: false);
                            });
                          ...
                          MockDatabase.updateUserVerificationStatus(user.id, true, 'phone');
                        }
                        ...
                        Navigator.pushReplacementNamed(
                          context, 
                          user.type == 'client' ? '/client_dashboard' : '/supplier_dashboard',
                        );
                      } else {
                        ...
                        setState(() {
                          ...
                          if (result['expired'] == true) {
                            ...
                            errorMsg = 'تم تجاوز الحد الأقصى لمحاولات التحقق. يرجى طلب رمز جديد.';
                            // إتاحة زر إعادة الإرسال على الفور
                            remainingSeconds = 0;
                          } else {
                            errorMsg = result['message'] ?? 'رمز التحقق غير صحيح.';
                            if (result['remainingAttempts'] != null) {
                              errorMsg += ' (محاولات متبقية: ${result['remainingAttempts']})';
                            }
                          }
                        });
                      }
